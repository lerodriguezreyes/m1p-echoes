window.addEventListener("load", () => {
  ...
  const resetButtonGO = document.querySelector(".restart1");
  const resetButtonGW = document.querySelector(".restart2");
  ...
  const roomDescription = [
    ...
    // Boss Battle Scene
    "Armed with knowledge, you gaze into the abyss and await to confront another monster. You yell into the darkness: Unknown Horror come face me if you dare! Next to you a grayish green portal opens.",
  ];
  ...
  function randomPlayerName() {
    ...
    let filteredPlayersArray = players.filter(
  ...
  const notesArray = [
    `If something can cross over from conceptual space into reality, taking physical form, then something can cross into the opposite direction. It must be possible to take a physical entity, mechanically extract the idea it embodies, amplify that idea, and broadcast it up into the conceptual space. A bigger idea, a universal code. One designed to fight this thing. ~ ${randomName}`,

    `Ok, ok so hear me out. The airlock configuration is not necessarily tied to a 24-hour cycle. If we really wanted to leave early all we must do is hot start the process. Maybe a controlled burst of arcane energy? Maybe a magic word? Ha! I got it:  Open Sesame! ~ ${randomName}`,

    `The monsters we are fighting obviously are not natural or real. We took time to dissect a couple before their bodies vanished. Its… strange. They have no organs, no systems. Only teeth, mouths, spiny projections of bone, teeth, and mouths. Did I say it has teeth? ~ ${randomName}`,

    `The protections surrounding the bunker are ingenious. The anti-divination effect is too powerful to maintain indefinitely. So, it is only designed to work periodically, say every 24 hours or so. Then the protection slowly recharges over time. That stored energy is then used to power it again. The cycle must obviously be synchronized with whatever is normally preventing us from discovering this bunker hidden in the basement in the first place. ~ ${randomName}`,

    `How do you fight an enemy without ever discovering it exists? How do you win without even realizing you’re at war? What do we do? ~ ${randomName}`,

    ` Ok, so apparently something out there is eating our memories and trying to keep us in the dark. I say to hell with it. Let us use what we have got here and scrap something together to disable the cloak and take the fight to it. We do not really understand what is going on, but maybe we could see what we are missing. Or at least… remember. ~ ${randomName} `,

    `I have double checked the wards lining the bunker, and I did not find any evidence of physical, technological, or arcane weakening. This is as solid and secure as it gets. Guess we must wait for the timer to run out… Nope X that idea. The thing is here again, we cannot stay here! Whatever it is, it’s getting smarter. It is finding us faster. It must be pushing in harder. ~ ${randomName}`,
  ];
  ...
  const rooms = [
    ...
    new Room(
      roomDescription[0],
      "Will you read the plaque?",
      ["Read it.", "Enter the vault"],
      0,
      0,
      roomImagesArray[0]
    ),
    ...
    new Room(
      roomDescription[1],
      "What will you do?",
      ["Inspect the room", "Touch the Orb"],
      1,
      15,
      roomImagesArray[1]
    ),
    ...
    new Room(
      ...
      ["Read the second page", "Keep exploring"],
      2,
      10,
      roomImagesArray[2]
    ),
    ...
    new Room(
      roomDescription[3],
      "There's got to be something here to explain what's going on. Will you search this room?",
      ["Yes", "No"],
      3,
      10,
      roomImagesArray[3]
    ...
    new Room(
      roomDescription[5],
      "I'm stuck here, might as well search for something useful. Search the room?",
      ["Yes", "No"],
      5,
      10,
      roomImagesArray[5]
  ...
  function startCountdown() {
    ...
    let minutes = Math.floor(game.timeRemaining / 60)
      .toString()
      .padStart(2, "0");
    ...
    timer = setInterval(() => {
      ...
      minutes = Math.floor(game.timeRemaining / 60)
        .toString()
        .padStart(2, "0");
  ...
  startGame.addEventListener("click", () => {
    ...
    choice1Button.addEventListener("click", () => {
      if (game.roomIndex == 0) {
        ...
        alert(
          `After searching for some time, you find nothing out of  the ordinary save for the Orb in the pedestal.`
        );
    ...
    choice2Button.addEventListener("click", () => {
      if (game.roomIndex == 0) {
        ...
        alert(
          `You touch the orb. At first nothing happens, then pain like you've never felt before slowly ices your veins. Oh the agony! You suffer -25 HP.`
        );
        game.playerLife -= 25;
        game.changeRoom(
          choicesContainerDiv,
